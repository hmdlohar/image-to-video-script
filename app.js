...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    console.log(`[Upload] Saving ${file.fieldname} to ${finalDir} (Session: ${sId})`);
...
io.on('connection', (socket) => {
  ...
  socket.on('start-generation', async (data) => {
    ...
    try {
      if (!fs.existsSync(audioDir)) throw new Error('Audio directory not found. Upload may have failed.');
      if (!fs.existsSync(srtDir)) throw new Error('SRT directory not found. Upload may have failed.');
      if (!fs.existsSync(imageDir)) throw new Error('Images directory not found. Upload may have failed.');
      ...
      const srtContent = fs.readFileSync(path.join(srtDir, srtFiles[0]), 'utf8');
      const outputName = `video_${sessionId}_${Date.now()}.mp4`;
      const outputPath = path.join(outputDir, outputName);

      socket.emit('progress', { status: 'starting', message: 'Initializing...' });
